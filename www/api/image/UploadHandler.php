...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        $strPath = "/../store/".(int)$_POST['target']."/image/";
        $this->options = array(
            'script_url' => $this->get_full_url().'/',
            'upload_dir' => dirname($this->get_server_var('SCRIPT_FILENAME')).$strPath,
            'upload_url' => $this->get_full_url().$strPath,
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function trim_file_name($name,
		...
		// add back image extension
		if ($ext)
		{
			$name .= ".{$ext}";
		}
		
		// append unique id to name
        $name = sha1(uniqid("", true)).".{$name}";
